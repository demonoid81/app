...
VuexI18nPlugin.install = function install (Vue, store, config) {
    ...
    const translateInLanguage = function translateInLanguage (locale) {
        ...
        const asyncTranslation = onTranslationNotFound(locale, key, defaultValue)
    ...
    const checkKeyExists = function checkKeyExists (key, scope = 'fallback') {
        // get the current language from the store
        const locale = store.state[moduleName].locale
        const fallback = store.state[moduleName].fallback
        const translations = store.state[moduleName].translations
    ...
    // set fallback locale
    const setFallbackLocale = function setFallbackLocale (locale) {
        store.dispatch({
    ...
    const getLocales = function getLocales () {
    ...
    const addLocale = function addLocale (locale, translations) {
    ...
    const replaceLocale = function replaceLocale (locale, translations) {
        return store.dispatch({
    ...
    const removeLocale = function removeLocale (locale) {
    ...
    const phaseOutExistsFn = function phaseOutExistsFn (locale) {
    ...
    const checkLocaleExists = function checkLocaleExists (locale) {
...
const renderFn = function (identifiers) {
    ...
    const render = function render (locale, translation, replacements = {}, pluralization = null) {
